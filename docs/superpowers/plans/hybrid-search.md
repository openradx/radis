...
When a new increment is planned, its executable plan is appended at the bottom
of this file and collapses into a history entry once executed (full plan text
...
## 9. Report-centric badge + backfill runs (planned 2026-07-16)

The badge leads with reports: global `N / M reports processed` with
queued / in progress / not queued breakdown, a `Backfill: P / T (X%)` line
backed by the new `EmbeddingBackfillRun` model (counter-based progress,
single active run enforced with abandoned-run auto-close, stall marker),
and subjob mechanics demoted to a muted secondary line. Cancellation and
the cancel-button gating became run-scoped (`args->'run_id'` of active
runs) after the final review found the old priority filter was defeated by
`retry_stalled_jobs` re-prioritization. Read-only run-history admin.
Commits `ea0ff81a` (model), `ad8de810` (plumbing), `fb6dd6e1` (entry
points), `c07a0c47` (badge), `fbda4476` (spec sync), `bb9ad526` +
`4e694079` (run-scoped cancel fix). Full task-level plan:
`git show 1770b6a1:docs/superpowers/plans/hybrid-search.md`.
