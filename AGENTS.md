...
Further server tuning is a deployment concern, not repo configuration. Two
that large embedded archives will hit (see the admin guide): raising
`maintenance_work_mem` for index rebuilds via
`ALTER SYSTEM SET ... ; SELECT pg_reload_conf();` (persists in the data
volume, no restart), and the container's `/dev/shm` — Docker's 64 MB default
is too small for parallel maintenance, and it cannot be changed from inside
PostgreSQL, so mount a larger tmpfs in your own compose override or via
