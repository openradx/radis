...
def test_extraction_wizard_step1_fill_fields(live_server: LiveServer, page: Page):
    ...
    expect(page.locator("#id_0-0-description")).to_have_value("The type of finding in the report")
...
@pytest.mark.order("last")
@pytest.mark.django_db(transaction=True)
def test_extraction_job_detail_shows_search_params(live_server: LiveServer, page: Page):
