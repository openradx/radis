...
from .utils.processor_utils import (
    generate_question_fields_prompt,
    generate_question_fields_schema,
)
...
class SubscriptionTaskProcessor(AnalysisTaskProcessor):
    ...
    def process_report(self, report: Report, task: SubscriptionTask) -> None:
        ...
        Schema = generate_question_fields_schema(subscription.question_fields)
        prompt = Template(settings.QUESTION_FIELDS_SYSTEM_PROMPT).substitute(
            {
                "report": report.body,
                "questions": generate_question_fields_prompt(subscription.question_fields),
            }
        )
