...
def test_create_subscription_with_filter_question(live_server: LiveServer, page: Page):
    ...
    page.locator("#id_filter_questions-0-question").fill("Does the report mention pneumonia?")
...
def test_create_subscription_with_output_field(live_server: LiveServer, page: Page):
    ...
    page.locator("#id_output_fields-0-description").fill("The size of the largest finding in mm")
...
@pytest.mark.order("last")
@pytest.mark.django_db(transaction=True)
def test_view_subscription_detail_shows_filter_questions(live_server: LiveServer, page: Page):
